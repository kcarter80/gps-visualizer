...
export async function ready() {
	status('Checking Strava login status.<br/>');
	if (await stravaLogin()) {
		status('Logged in to Strava.<br/> Loading Strava activities.<br/>');
		const activity_data = await getStravaActivities();
		status('Activities loaded.<br/>');
		...
		status('Waiting for user to select activities ➡️ and to generate animation ⬇️.');		
	} else {
